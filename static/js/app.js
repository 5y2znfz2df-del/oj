...
async function submitCode(pid) {
  ...
  panel.innerHTML = '<div class="console-title">📟 控制台 / 编译信息</div><div class="console-body"><div class="empty">⌛ 判题中，婆罗门正在一刻不停地搬盘子…</div></div>';
  ...
  try {
    ...
    panel.innerHTML = `
      ...
      <div class="console-body">
        ...
        ${r.detail ? `<pre class="console-out">${escapeHtml(r.detail)}</pre>` : ''}
        ${r.status === 'AC' ? '<div class="console-ac">🎉 AC！+10 积分到手（首 AC）。</div>' : ''}
      </div>`;
    if (r.status === 'AC') showAcAnimation();
    await refreshUser();
  } catch (e) {
    panel.innerHTML = '<div class="console-title">📟 控制台 / 编译信息</div><div class="console-body"><div class="console-out" style="color:#f87171">' + escapeHtml(e.message) + '</div></div>';
  } finally {
